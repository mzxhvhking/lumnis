	...
	while (pClass)
	{
		...
		if (!strcmp(pClass->m_pNetworkName, xorstr("CBaseViewModel")))
		{
			for (int i = 0; i < pClass->m_pRecvTable->m_nProps; i++)
			{
				...
				if (!strcmp(name, xorstr("m_nSequence")))
				{
					oSequenceProxyFn = (RecvVarProxyFn)pProp->m_ProxyFn;
					pProp->m_ProxyFn = RecvProxy_SetViewModelSequence;
				}
			}
		}
	...
	while (pClass)
	{
		...
		if (!strcmp(pClass->m_pNetworkName, xorstr("CBaseViewModel")))
		{
			for (int i = 0; i < pClass->m_pRecvTable->m_nProps; i++)
			{
				...
				if (!strcmp(name, xorstr("m_nSequence")))
				{
					pProp->m_ProxyFn = oSequenceProxyFn;
				}
			}
		}
